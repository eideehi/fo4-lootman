...
describe("FOMOD release policy", () => {
	it("states the 3.3.0 update compatibility policy", () => {
		const notice = getCompatibilityNotice(readModuleConfig());

		expect(notice).toContain('group name="About Compatibility"');
		expect(notice).toContain('plugin name="LootMan 3.3.0 update compatibility"');
		expect(notice).toContain('<flag name="compatibility_notice_read">selected</flag>');
		expect(notice).toContain(
			"LootMan 3.3.0 supports overwrite updates from LootMan 2.x and earlier 3.x releases.",
		);
		expect(notice).toContain("LootMan 3.3.0 does not support overwrite updates from LootMan 1.x.");
		expect(notice).toContain("If you are upgrading from 1.x, uninstall 1.x and make a clean save before installing 3.3.0.");
	...
	it("targets the supported Fallout 4 runtime", () => {
		...
		expect(moduleConfig).toContain('<plugin name="1.11.240">');
		expect(moduleConfig).toContain('<flag name="runtime_1_11_240">selected</flag>');
		expect(moduleConfig).toContain("supports only Fallout4.exe 1.11.240");
		expect(moduleConfig).toContain("version-1-11-240-0.bin");
		expect(moduleConfig).toContain("will not load on 1.11.221, 1.10.163, or other runtimes");
		expect(moduleConfig).not.toContain('<plugin name="1.11.221">');
		expect(moduleConfig).not.toContain("runtime_1_11_221");
		expect(moduleConfig).not.toContain('<plugin name="1.10.163">');
